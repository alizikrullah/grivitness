...
export const useAuthStore = create<AuthState>((set) => {
  const terima = async (hasil: AuthResult): Promise<void> => {
    await saveTokens({ access: hasil.access_token, refresh: hasil.refresh_token });
  ...
  // aplikasi akan diam di layar kosong dengan token yang sudah mati.
  ...
  return {
    ...
    login: async (email, password) => {
      await terima(await post<AuthResult>('/api/auth/login', { email, password }));
    },

    register: async (name, email, password) => {
      await terima(await post<AuthResult>('/api/auth/register', { name, email, password }));
    },
